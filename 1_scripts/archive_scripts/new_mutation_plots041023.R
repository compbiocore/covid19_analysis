...
ridata <- read_csv("/gpfs/data/ris3/dev/20230312/3_results/20230313/qc-passed.csv") %>%
...
concernMuts <- read.table("/gpfs/data/ris3/dev/20230312/2_metadata/mutations_of_concern.txt", sep="\t", header=TRUE) %>% # table is all lineages copied from https://www.ecdc.europa.eu/en/covid-19/variants-concern with headers manually assigned for ease
...
write.table(concernMuts, file = "concernMuts.txt", sep = "\t",
            row.names = FALSE)
